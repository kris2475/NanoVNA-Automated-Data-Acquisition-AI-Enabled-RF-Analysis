...
import time
import csv
import sys # Import sys for exiting if input fails

# ====================================================================
# SCRIPT CONFIGURATION
# Adjust these values to set the frequency range and resolution.
# The number of sweeps is now determined by user input.
# ====================================================================
...
# ====================================================================
# USER INPUT FOR SWEEPS
# ====================================================================
def get_num_sweeps():
    """Prompts the user for the number of sweeps and validates the input."""
    while True:
        try:
            # Prompt the user
            sweeps_str = input("Please enter the total number of sweeps (e.g., 5): ")
            NUM_SWEEPS = int(sweeps_str)

            if NUM_SWEEPS <= 0:
                print("Error: The number of sweeps must be a positive integer.")
            else:
                return NUM_SWEEPS
        except ValueError:
            print("Error: Invalid input. Please enter a whole number.")
        except EOFError:
            # Handle case where input is closed (e.g., running from a non-interactive shell)
            print("Input terminated unexpectedly. Exiting.")
            sys.exit(1)
...
def s_to_mag_phase(s_complex):
    """Converts complex S-parameter to Magnitude (dB) and Phase (degrees)."""
...
# 1. Get the desired number of sweeps from the user
NUM_SWEEPS = get_num_sweeps()
print(f"Set to perform {NUM_SWEEPS} total sweeps.")

vna = None # Initialize vna variable

try:
    # 2. Initialize and Connect to the NanoVNA
    vna = pynanovna.VNA()
    print("Connection successful.")

    # 3. Set Sweep Parameters once for the entire batch
    ...
    # 4. Loop through the desired number of sweeps
